...
public abstract class BenchMark {
    ...
    public static int ITERATIONS = 100000; //original value
	

	// startup
	public int RunStartup() {
		BenchMark self = this;
		return self.runOneIteration();
	}
    ...
    {
        outStream.printf("Invalid number of arguments {0}. Usage:", argumentsCount);
        outStream.printf("'{0} it' for Startup", Program.FRIENDLY_NAME); //(single bench iteration)
        outStream.printf("'{0} it n k Cov' for Steady", Program.FRIENDLY_NAME); // (n=maxIterations, k=measurementsPerInvocation, CoV=coefficientOfVariation)
    }
