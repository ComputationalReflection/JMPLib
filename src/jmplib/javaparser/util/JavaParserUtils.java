 ...
 * 
 * @author Ignacio Lagartos, José Manuel Redondo
...
public class JavaParserUtils {
		...
		for (BodyDeclaration member : members) {
			if (member instanceof MethodDeclaration) {
				...
				for (int i = 0;i< parameterClasses.length; i++)
					System.out.print(parameterClasses[i] + ", ");
	...
	private static final String [] primitiveTypeNames = {"byte", "short","int", "double", "long", "String", "float", "boolean", "char"};
	...
	private static boolean checkParameterClasses(MethodDeclaration method, List<ImportDeclaration> imports,
		...
		for (int i = 0; i < parameterClasses.length; i++) {
			//System.out.println(parameters[i] + " is a type parameter " + isATypeParameter(method, parameters[i]) );
	...
	private static boolean checkReturnClass(MethodDeclaration method, List<ImportDeclaration> imports,
		...
		//System.out.println(method.getType() + ": " + isATypeParameter(method, method.getType()));
