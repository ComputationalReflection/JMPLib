...
public abstract class AbstractPrimitive implements Primitive {
     ...
     *
     * @param clazz          The class to evolve
     * @param superclassName The new superclass
     * @throws StructuralIntercessionException
     */
    protected void updateVersion(Class<?> clazz, String superclassName)
            throws StructuralIntercessionException {
     ...
     *
     * @param clazz          The class to revert
     * @param superclassName The old superclass name
     * @throws StructuralIntercessionException
     */
     ...
     * @param from    Actual version number of the source code
     * @param to      New version number
     * @return Source code updated
     */
    protected String changeVersion(String content, Class<?> clazz, int from,
                                   int to) {
        // Update the className
    ...
    protected Class<?> getSuperClass(ClassContent classContent) throws StructuralIntercessionException {
        // Obtain the class
...
/*		
		...
		for (TypeDeclaration t: types) {
			...
			System.out.println(classContent.getClazz().getName());
			
			if (t.getName().equals(classContent.getClazz().getName())) {
				return t.getClass().getSuperclass();
			}
		}*/
        return unit.getClass().getSuperclass();
    }

    /**
     * Obtains the final superclass
     *
     * @param classContent The {@link ClassContent} of the class
    ...
    private ClassContent getTopSuperClass(ClassContent classContent) {
        ...
        ClassContent superclassContent = null;
        try {
            if (changesClassHierarchy) {
                if (getSuperClass(classContent) == null)
                    return classContent;
                superclassContent = SourceCodeCache.getInstance().getClassContent(
                        getSuperClass(classContent));
            } else {
                if (classContent.getClazz().getSuperclass() == null)
                    return classContent;
                superclassContent = SourceCodeCache.getInstance().getClassContent(
     ...
     * funtionality.
     *
     * @throws StructuralIntercessionException If there are any error
     */
