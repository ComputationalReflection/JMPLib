...
public class ReplaceMethodPrimitive extends MethodPrimitive {
    ...
    public ReplaceMethodPrimitive(ClassContent classContent, String name, String body, Class<?> returnType,
                                  Class<?>[] parameterClasses, Class<?> newReturnType, Class<?>[] newParameterClasses) {
    ...
    private void initialize() throws StructuralIntercessionException {
        if (parameterClasses.length != newParameterClasses.length)
            throw new StructuralIntercessionException("The number of parameters have to match");
        obtainMethodData();
        createBody();
        generateMethod(body);
        generateInvoker();
     ...
     *
     * @param body new body of the method
     * @throws StructuralIntercessionException
     */
    private void generateMethod(String body) throws StructuralIntercessionException {
     ...
     * @throws StructuralIntercessionException
     */
    private void generateParams(String name, Type returnType) throws StructuralIntercessionException {
        List<Parameter> parameters = new ArrayList<Parameter>();
        for (int i = 0; i < newParameterClasses.length; i++) {
            parameters.add(
                    new Parameter(JavaParserUtils.transform(newParameterClasses[i]), this.parameters.get(i).getId()));
        }
        declaration = new MethodDeclaration(modifiers, returnType, name, parameters);
    }

    /**
     * Parses the body code
     *
     * @param body Code of the body
    ...
    private void generateInvoker() throws StructuralIntercessionException {
        ...
        invoker = new MethodDeclaration(ReplaceMethodPrimitive.MODIFIERS, returnType, "_" + name + "_invoker",
                parameter);
    ...
    private void obtainMethodData() throws StructuralIntercessionException {
        ...
        try {
            declaration = JavaParserUtils.searchMethod(unit, clazz, name, parameterClasses, returnClass);
    ...
    /**
     * Creates the new body for the old method, this body will redirect the calls to
     * the new method
     *
     * @throws StructuralIntercessionException
     */
